...
$(function(){
	var vs = new Array();
	vs = ${version}
	console.log(vs.versions[0].id)
	console.log(vs.versions.length)             
	
	$.each(vs.versions, function(idx, versions){
		var $tu = $("<tr></tr>").appendTo($("#versionsTable"))
		$("<td></td>").text(versions.id).appendTo($tu)
		$("<td></td>").text(versions.project.id).appendTo($tu)
		$("<td></td>").text(versions.project.name).appendTo($tu)
		$("<td></td>").text(versions.name).appendTo($tu)
		$("<td></td>").text(versions.status).appendTo($tu)
		$("<td></td>").text(versions.due_date).appendTo($tu)
		$("<td></td>").text(versions.sharing).appendTo($tu)
		$("<td></td>").text(versions.created_on).appendTo($tu)
		$("<td></td>").text(versions.updated_on).appendTo($tu)
              ...
              <div class=" form">
                <form class="cmxform form-horizontal style-form" id="commentForm" method="POST" action="createVersions">
                 ...
                 <!--  <div class="form-group ">
                   ...
                   <div class="form-group ">
                    <label for="cemail" class="control-label col-lg-2">버전 이름 (required)</label>
                    <div class="col-lg-10">
                      <input class="form-control " id="cemail" type="text" name="vername" required />
                    </div>
                  </div>
               	<!--  <div class="form-group ">
